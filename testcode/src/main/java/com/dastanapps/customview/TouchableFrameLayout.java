        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_DOWN:
                if(event.getPointerCount()>=2){
                    oldDist = (float) Math.sqrt(Math.pow(event.getX(0) - event.getX(1), 2) + Math.pow(event.getY(0) - event.getY(1), 2));
                }
                    ...
                    if (onScaleMove) {
                        ...
                        if ((Math.abs(dist - lastdist) > 10) && (lastdist > 0) && (dist > 0)) {
                            ...
                            if (dist == lastdist) {
                                ...
                                PointF newDist = new PointF();
                                newDist.x = event.getX(0) - event.getX(1);
                                newDist.y = event.getY(0) - event.getY(1);
                                double newD = dist;
                                float scale = (float) (newD / oldDist);

                                onTouchListener.scale(scale, scale);
                            }
                        } else {
