...
void android_log_print(const char *tag, const char *fmt, ...);

#define printf(...) android_log_print(LOG_TAG, __VA_ARGS__);
...
int run_probe(int argc, char **argv, ProbeCallback callback) {
    output_callback = callback.ouput_callback;
    const Writer *w;
    WriterContext *wctx;
    char *buf;
