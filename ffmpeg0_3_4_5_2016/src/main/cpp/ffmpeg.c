    ...
    if (!ist->wrap_correction_done && is->start_time != AV_NOPTS_VALUE &&
        ...
        if (ist->next_dts == AV_NOPTS_VALUE
            ...
            if (new_start_time > is->start_time) {
                LOGD(TAG, "Correcting start time by %"
                        PRId64
...
void set_progress_callback(void (*callback)(char *)) {
...
int run(int argc, char **argv, Callback callback) {
    ...
    int ret;
    int64_t ti;
    set_progress_callback(callback.progress_callback);
...
//     }
    ...
    if (do_benchmark) {
        ...
        char *bench = malloc(100 * sizeof(char));
        sprintf(bench, "bench: utime=%0.3fs\n", ti / 1000000.0);
        callback.benchmark_callback(bench);
    }
    LOGD(TAG, "%"
            PRIu64
