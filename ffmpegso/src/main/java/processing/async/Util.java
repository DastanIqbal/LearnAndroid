...
public class Util {
    ...
    public static Map<Integer, String> getVideoData(String path, int... selectedKeys) {
        Map<Integer, String> resultMap = new HashMap<>();
        try {
            MediaMetadataRetriever mediaMetadataRetriever = new MediaMetadataRetriever();
            mediaMetadataRetriever.setDataSource(path);
            for (int key : selectedKeys) {
                resultMap.put(key, mediaMetadataRetriever.extractMetadata(key));
            }
        } catch (RuntimeException ex) {
            // Do nothing. Just let it return empty map.
        }
        return resultMap;
    }
}
